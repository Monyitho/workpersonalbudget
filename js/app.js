...
// Tab switch per categorie
function switchCatTab(tab) {
  var tabEntrate = document.getElementById('catTabEntrate');
  var tabUscite = document.getElementById('catTabUscite');
  var vistaEntrate = document.getElementById('catVistaEntrate');
  var vistaUscite = document.getElementById('catVistaUscite');

  if (tab === 'entrate') {
    tabEntrate.style.background = 'linear-gradient(135deg,var(--income),#3cb371)';
    tabEntrate.style.color = '#fff';
    tabUscite.style.background = 'transparent';
    tabUscite.style.color = 'var(--text)';
    vistaEntrate.style.display = 'block';
    vistaUscite.style.display = 'none';
  } else {
    tabUscite.style.background = 'linear-gradient(135deg,var(--expense),#e57373)';
    tabUscite.style.color = '#fff';
    tabEntrate.style.background = 'transparent';
    tabEntrate.style.color = 'var(--text)';
    vistaEntrate.style.display = 'none';
    vistaUscite.style.display = 'block';
  }
}

function mostraCats() {
  // Mostra categorie entrate - nuova grafica compatta
  var containerIn = document.getElementById('catIn');
  var htmlIn = '';

  DB.categorie.income.sort().forEach(function(cat, i) {
    htmlIn += '<div style="background:linear-gradient(135deg,rgba(40,167,69,0.1),rgba(40,167,69,0.05));border:1px solid rgba(40,167,69,0.3);border-radius:10px;padding:10px;display:flex;flex-direction:column;gap:6px">';
    htmlIn += '<div style="font-weight:700;font-size:0.85em;color:var(--income);overflow:hidden;text-overflow:ellipsis;white-space:nowrap">' + cat + '</div>';
    htmlIn += '<div style="display:flex;gap:4px">';
    htmlIn += '<button onclick="editCat(\'income\',' + i + ')" style="flex:1;padding:6px;background:var(--card);border:1px solid var(--border);border-radius:6px;font-size:0.8em;cursor:pointer">✏️</button>';
    htmlIn += '<button onclick="elimCat(\'income\',' + i + ')" style="flex:1;padding:6px;background:rgba(220,53,69,0.1);border:1px solid rgba(220,53,69,0.3);border-radius:6px;font-size:0.8em;cursor:pointer;color:var(--expense)">🗑️</button>';
    htmlIn += '</div></div>';
  });

  if (DB.categorie.income.length === 0) {
    htmlIn = '<div style="grid-column:1/-1;text-align:center;padding:20px;color:var(--text-muted);font-size:0.85em">Nessuna categoria entrate.<br>Clicca "+ Entrata" per aggiungerne una!</div>';
  }

  containerIn.innerHTML = htmlIn;

  // Aggiorna contatore
  var countEl = document.getElementById('catEntrateCount');
  if (countEl) countEl.textContent = DB.categorie.income.length;
  ...
  // Necessità - nuova grafica
  var containerNec = document.getElementById('catNecessita');
  var htmlNec = '';

  if (categorieNecessita.length === 0) {
    htmlNec = '<div style="grid-column:1/-1;text-align:center;padding:15px;color:var(--text-muted);font-size:0.8em;background:rgba(230,126,34,0.05);border-radius:8px">Nessuna Necessità</div>';
  } else {
    categorieNecessita.sort().forEach(function(cat) {
      var idx = DB.categorie.expense.indexOf(cat);
      htmlNec += '<div style="background:linear-gradient(135deg,rgba(230,126,34,0.1),rgba(230,126,34,0.05));border:1px solid rgba(230,126,34,0.3);border-radius:10px;padding:8px;display:flex;flex-direction:column;gap:5px">';
      htmlNec += '<div style="font-weight:700;font-size:0.8em;color:#d35400;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">' + cat + '</div>';
      htmlNec += '<div style="display:flex;gap:3px">';
      htmlNec += '<button onclick="editCat(\'expense\',' + idx + ')" style="flex:1;padding:5px;background:var(--card);border:1px solid var(--border);border-radius:5px;font-size:0.75em;cursor:pointer">✏️</button>';
      htmlNec += '<button onclick="elimCat(\'expense\',' + idx + ')" style="flex:1;padding:5px;background:rgba(220,53,69,0.1);border:1px solid rgba(220,53,69,0.3);border-radius:5px;font-size:0.75em;cursor:pointer">🗑️</button>';
      htmlNec += '<button onclick="cambiaClassificazione(\'' + cat + '\', \'desideri\')" style="flex:1.5;padding:5px;background:#9b59b6;color:#fff;border:none;border-radius:5px;font-size:0.65em;cursor:pointer;font-weight:600">→💎</button>';
      htmlNec += '</div></div>';
    });
  }
  ...
  // Aggiorna contatore necessita
  var countNecEl = document.getElementById('catNecessitaCount');
  if (countNecEl) countNecEl.textContent = categorieNecessita.length;

  // Desideri - nuova grafica
  var containerDes = document.getElementById('catDesideri');
  var htmlDes = '';

  if (categorieDesideri.length === 0) {
    htmlDes = '<div style="grid-column:1/-1;text-align:center;padding:15px;color:var(--text-muted);font-size:0.8em;background:rgba(155,89,182,0.05);border-radius:8px">Nessun Desiderio</div>';
  } else {
    categorieDesideri.sort().forEach(function(cat) {
      var idx = DB.categorie.expense.indexOf(cat);
      htmlDes += '<div style="background:linear-gradient(135deg,rgba(155,89,182,0.1),rgba(155,89,182,0.05));border:1px solid rgba(155,89,182,0.3);border-radius:10px;padding:8px;display:flex;flex-direction:column;gap:5px">';
      htmlDes += '<div style="font-weight:700;font-size:0.8em;color:#8e44ad;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">' + cat + '</div>';
      htmlDes += '<div style="display:flex;gap:3px">';
      htmlDes += '<button onclick="editCat(\'expense\',' + idx + ')" style="flex:1;padding:5px;background:var(--card);border:1px solid var(--border);border-radius:5px;font-size:0.75em;cursor:pointer">✏️</button>';
      htmlDes += '<button onclick="elimCat(\'expense\',' + idx + ')" style="flex:1;padding:5px;background:rgba(220,53,69,0.1);border:1px solid rgba(220,53,69,0.3);border-radius:5px;font-size:0.75em;cursor:pointer">🗑️</button>';
      htmlDes += '<button onclick="cambiaClassificazione(\'' + cat + '\', \'necessita\')" style="flex:1.5;padding:5px;background:#e67e22;color:#fff;border:none;border-radius:5px;font-size:0.65em;cursor:pointer;font-weight:600">→🎯</button>';
      htmlDes += '</div></div>';
    });
  }
  ...
  // Aggiorna contatore desideri
  var countDesEl = document.getElementById('catDesideriCount');
  if (countDesEl) countDesEl.textContent = categorieDesideri.length;
}
