...
function aggiornaCascataMensile(entrate, usciteReali, cashConto, daRecuperareCumulativo, mieUscite) {
  ...
  // Splitwise row — mostra saldo CUMULATIVO (il vero saldo Splitwise con il partner)
  var saldoDaMostrare = (daRecuperareCumulativo !== undefined) ? daRecuperareCumulativo : splitwiseMese;
  elSplitLbl.textContent = 'Saldo con ' + pName;
  if (saldoDaMostrare > 0.01) {
    elSplit.textContent = '+' + formatEuro(saldoDaMostrare);
    elSplit.style.color = '#4ecca3';
    elSplitSub.textContent = pName + ' ti deve (totale)';
  } else if (saldoDaMostrare < -0.01) {
    elSplit.textContent = '−' + formatEuro(Math.abs(saldoDaMostrare));
    elSplit.style.color = '#fca5a5';
    elSplitSub.textContent = 'devi a ' + pName + ' (totale)';
  } else {
...
function aggiornaAnalisiRisparmio(entrate, speseNecessita, speseDesideri, risparmioSulConto, daRecuperare, risparmioTotale, budgetNecessita, budgetDesideri, budgetRisparmi, totaleUscite) {
  ...
  // Entrate
  html += '<div class="stat-flat-row c-green">';
  html += '<div><div class="stat-flat-label">Entrate</div><div class="stat-flat-sub">Stipendio e altre entrate</div></div>';
  html += '<div class="stat-flat-val c-green">+' + formatEuro(entrate) + '</div>';
  html += '</div>';

  // Le Mie Spese
  html += '<div class="stat-flat-row c-red">';
  html += '<div><div class="stat-flat-label">Le Mie Spese</div><div class="stat-flat-sub">Personali + metà condivisa</div></div>';
  html += '<div class="stat-flat-val c-red">−' + formatEuro(totaleUscite) + '</div>';
  html += '</div>';

  // Separatore =
  html += '<div style="border-top:1px dashed rgba(255,255,255,0.1);margin:10px 0"></div>';

  // Risparmio effettivo (numero principale)
  var rispEffColor = risparmioEffettivo >= 0 ? '#27ae60' : '#e74c3c';
  html += '<div class="total-strip">';
  html += '<div><div class="ts-label">= Risparmio del Mese</div><div class="ts-sub">Entrate − Le Mie Spese</div></div>';
  html += '<div class="ts-val" style="color:' + rispEffColor + '">' + formatEuro(risparmioEffettivo) + '</div>';
  html += '</div>';

  // Separatore
  html += '<div style="border-top:1px dashed rgba(255,255,255,0.1);margin:10px 0"></div>';

  // Da recuperare / rimborsare / pari (info aggiuntiva sul partner questo mese)
  var daRecupClass = 'c-orange';
  var daRecupLabel = 'Pareggio con partner';
  var daRecupSub = 'Questo mese siete pari';
  if (daRecuperare > 0.01)  { daRecupClass = 'c-green'; daRecupLabel = 'Da Recuperare (mese)'; daRecupSub = 'Partner ti deve questo mese'; }
  else if (daRecuperare < -0.01) { daRecupClass = 'c-red'; daRecupLabel = 'Da Rimborsare (mese)'; daRecupSub = 'Tu devi al partner questo mese'; }
