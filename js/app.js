...
function aggiornaCondivisoTotale() {
  ...
  // MODIFICATO: AGGIUNGI anticipi partner (hai pagato per il partner, quindi devi recuperare di più)
  var daRecReale = daRecBase - prestiti + virtualRecoveryTu - virtualRecoveryPartner + anticipiPartner;
...
function mostraLongTermGoals() {
  ...
  if (!DB.longTermGoals || DB.longTermGoals.length === 0) {
    ...
    html += '<p style="color:#7f8c8d;margin-bottom:15px">Crea obiettivi su più mesi per raggiungere traguardi importanti!</p>';
    html += '</div>';
    container.innerHTML = html;
    return;
  }
  ...
  DB.longTermGoals.forEach(function(goal, idx) {
    ...
    // Info periodo
    var mesiNomi = ['Gen','Feb','Mar','Apr','Mag','Giu','Lug','Ago','Set','Ott','Nov','Dic'];
    var periodoText = mesiNomi[dataInizio.getMonth()] + ' ' + dataInizio.getFullYear() + ' → ' + mesiNomi[dataFine.getMonth()] + ' ' + dataFine.getFullYear();
    html += '<div style="font-size:0.8em;color:#7f8c8d;margin-top:2px">' + periodoText + ' • ' + emoji + ' ' + stato + '</div>';
    html += '</div>';
    html += '<div style="display:flex;gap:6px">';
    html += '<button onclick="eliminaLongTermGoal(' + idx + ')" style="background:#e74c3c;color:#fff;border:none;padding:8px 12px;border-radius:8px;cursor:pointer;font-size:0.9em;font-weight:600;box-shadow:0 2px 4px rgba(231,76,60,0.3)">🗑️</button>';
    html += '</div>';
    html += '</div>';
    
    // Importi (layout verticale per mobile)
    html += '<div style="display:flex;flex-direction:column;gap:8px;margin-bottom:12px">';

    if (goal.tipo === 'risparmio') {
      // Risparmiato
      html += '<div style="display:flex;justify-content:space-between;align-items:center;padding:10px 12px;background:rgba(76,175,80,0.15);border-radius:8px">';
      html += '<span style="font-size:0.9em;color:#2e7d32;font-weight:600">💵 Risparmiato</span>';
      html += '<span style="font-size:1.1em;font-weight:800;color:#2e7d32">' + formatEuro(speso) + '</span>';
      html += '</div>';
      // Obiettivo
      html += '<div style="display:flex;justify-content:space-between;align-items:center;padding:10px 12px;background:rgba(33,150,243,0.15);border-radius:8px">';
      html += '<span style="font-size:0.9em;color:#1976d2;font-weight:600">🎯 Obiettivo</span>';
      html += '<span style="font-size:1.1em;font-weight:800;color:#1976d2">' + formatEuro(target) + '</span>';
      html += '</div>';
      // Mancante
      html += '<div style="display:flex;justify-content:space-between;align-items:center;padding:10px 12px;background:rgba(255,152,0,0.15);border-radius:8px">';
      html += '<span style="font-size:0.9em;color:#e65100;font-weight:600">💰 Mancante</span>';
      html += '<span style="font-size:1.1em;font-weight:800;color:' + (rimanente >= 0 ? '#e65100' : '#e74c3c') + '">' + formatEuro(Math.abs(rimanente)) + '</span>';
      html += '</div>';
    } else if (goal.tipo === 'riduzione') {
      // Riduzione
      html += '<div style="display:flex;justify-content:space-between;align-items:center;padding:10px 12px;background:rgba(76,175,80,0.15);border-radius:8px">';
      html += '<span style="font-size:0.9em;color:#2e7d32;font-weight:600">📉 Riduzione</span>';
      html += '<span style="font-size:1.1em;font-weight:800;color:#2e7d32">' + speso.toFixed(1) + '%</span>';
      html += '</div>';
      // Obiettivo
      html += '<div style="display:flex;justify-content:space-between;align-items:center;padding:10px 12px;background:rgba(33,150,243,0.15);border-radius:8px">';
      html += '<span style="font-size:0.9em;color:#1976d2;font-weight:600">🎯 Obiettivo</span>';
      html += '<span style="font-size:1.1em;font-weight:800;color:#1976d2">' + target + '%</span>';
      html += '</div>';
      // Categoria
      html += '<div style="display:flex;justify-content:space-between;align-items:center;padding:10px 12px;background:rgba(156,39,176,0.15);border-radius:8px">';
      html += '<span style="font-size:0.9em;color:#7b1fa2;font-weight:600">📂 Categoria</span>';
      html += '<span style="font-size:1em;font-weight:700;color:#7b1fa2">' + goal.categoria + '</span>';
      html += '</div>';
    } else if (goal.tipo === 'budget_multi') {
      // Speso
      html += '<div style="display:flex;justify-content:space-between;align-items:center;padding:10px 12px;background:rgba(244,67,54,0.15);border-radius:8px">';
      html += '<span style="font-size:0.9em;color:#c62828;font-weight:600">💸 Speso</span>';
      html += '<span style="font-size:1.1em;font-weight:800;color:#c62828">' + formatEuro(speso) + '</span>';
      html += '</div>';
      // Budget
      html += '<div style="display:flex;justify-content:space-between;align-items:center;padding:10px 12px;background:rgba(33,150,243,0.15);border-radius:8px">';
      html += '<span style="font-size:0.9em;color:#1976d2;font-weight:600">🎯 Budget</span>';
      html += '<span style="font-size:1.1em;font-weight:800;color:#1976d2">' + formatEuro(target) + '</span>';
      html += '</div>';
      // Rimanente
      html += '<div style="display:flex;justify-content:space-between;align-items:center;padding:10px 12px;background:rgba(76,175,80,0.15);border-radius:8px">';
      html += '<span style="font-size:0.9em;color:#2e7d32;font-weight:600">✅ Rimanente</span>';
      html += '<span style="font-size:1.1em;font-weight:800;color:' + (rimanente >= 0 ? '#2e7d32' : '#e74c3c') + '">' + formatEuro(Math.abs(rimanente)) + '</span>';
      html += '</div>';
    }
...
function creaLongTermGoal(tipo) {
  ...
  if (tipo === 'risparmio') {
    ...
    var html = '';
    html += '<div class="modal-header">';
    html += '<h3 style="display:flex;align-items:center;gap:10px"><span style="font-size:1.4em">💰</span> Nuovo Obiettivo Risparmio</h3>';
    html += '<button class="close-btn" onclick="chiudiModal()">×</button>';
    html += '</div>';
    html += '<div style="padding:25px">';
    ...
    html += '<div style="font-size:0.85em;color:#7f8c8d;margin-top:6px;padding-left:4px">💡 Scegli un nome motivante!</div>';
    html += '</div>';
    ...
    html += '<span style="position:absolute;left:14px;top:14px;font-size:1em;color:#27ae60;font-weight:700">€</span>';
    html += '</div>';
    html += '</div>';
    ...
    html += '<button class="btn" onclick="salvaRisparmioGoal()" style="flex:2;background:#27ae60;padding:14px;font-weight:700">✅ Crea Obiettivo</button>';
    ...
    // Data fine
    html += '<div style="margin-bottom:20px">';
    html += '<label style="display:block;font-weight:700;margin-bottom:8px;color:var(--text);font-size:0.95em">📅 Entro quale data?</label>';
    ...
    html += '<button class="btn" onclick="salvaRiduzioneGoal()" style="flex:2;background:linear-gradient(135deg,#e74c3c,#c0392b);padding:14px;border-radius:10px;border:none;color:#fff;font-weight:700;font-size:1em;box-shadow:0 4px 12px rgba(231,76,60,0.3)">✅ Crea Obiettivo</button>';
    html += '</div>';
    html += '</div>';
    ...
    html += '<h3 style="display:flex;align-items:center;gap:10px;margin:0;font-size:1.3em"><span style="font-size:1.2em">🎯</span> Budget Multi-Mese</h3>';
    html += '<button class="close-btn" onclick="chiudiModal()" style="background:rgba(255,255,255,0.2);color:#fff;border:none;width:32px;height:32px;border-radius:50%;font-size:1.5em;cursor:pointer;display:flex;align-items:center;justify-content:center">×</button>';
    html += '</div>';
    html += '<div style="padding:20px;background:var(--card)">';
    ...
    html += '<div style="position:relative">';
    html += '<span style="position:absolute;left:12px;top:12px;font-size:1em;color:#9b59b6;font-weight:700">€</span>';
    ...
    // Data fine
    html += '<div style="margin-bottom:20px">';
    html += '<label style="display:block;font-weight:700;margin-bottom:8px;color:var(--text);font-size:0.95em">📅 Fino a quale data?</label>';
    ...
    html += '<div style="display:flex;gap:10px">';
    html += '<button class="btn" onclick="chiudiModal()" style="flex:1;background:#95a5a6;padding:14px;border-radius:10px;border:none;color:#fff;font-weight:700;font-size:1em">Annulla</button>';
    html += '<button class="btn" onclick="salvaBudgetMultiGoal()" style="flex:2;background:linear-gradient(135deg,#9b59b6,#8e44ad);padding:14px;border-radius:10px;border:none;color:#fff;font-weight:700;font-size:1em;box-shadow:0 4px 12px rgba(155,89,182,0.3)">✅ Crea Obiettivo</button>';
    html += '</div>';
    html += '</div>';
...
function salva(e) {
  ...
  try {
    ...
    console.log('[SALVA] ID:', id, 'Data:', dataTransazione);
  ...
  if (id === '') {
    ...
    var idx = DB.transazioni.findIndex(function(tr) { return tr.id == id; });
    if (idx !== -1) {
      t.id = DB.transazioni[idx].id;
...
function mostraTrans() {
  ...
  if (listaFiltrata.length === 0) {
    ...
    listaFiltrata.forEach(function(t) {
      ...
      var metodoPagamento = t.metodo === 'contanti' ? '💵 Contanti' : '💳 Carta/Conto';
...
function modifica(id) {
  console.log('🔧 MODIFICA CHIAMATA - Indice array:', id);
  ...
  document.getElementById('modal').classList.add('active');
  // FIX: Salva l'ID reale della transazione, non l'indice array
...
function checkMetodoPagamento() {
  ...
  if (metodo === 'buoni_pasto' && !mistoCheckbox.checked) {
    // Suggerimento visivo
...
function checkDestinazioneSelezionata() {
  ...
  if (dest.value === 'buoni_pasto') {
    ...
    // NUOVO - Auto-seleziona categoria "Buoni Pasto" e disabilita
    if (categoriaSelect) {
      categoriaSelect.value = 'Buoni Pasto';
      categoriaSelect.disabled = true;
      categoriaSelect.style.opacity = '0.6';
      categoriaSelect.style.cursor = 'not-allowed';
      categoriaSelect.style.background = '#ecf0f1';
    }
    
    // Disabilita campo importo (calcolato auto)
    timpInput.readOnly = true;
    timpInput.style.background = '#ecf0f1';
