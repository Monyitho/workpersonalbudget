...
function vai(sec, elem) {
  ...
  if (sec === 'settings') aggiornaVisibilitaBuoniSettings(); // NUOVO: Aggiorna visibilità buoni
  if (sec === 'finanze') {
...
function aggCatSel() {
  ...
  if (tipo === 'income') {
    ...
    var condivisoGroup = document.querySelector('.form-group:has(#tcondiviso)');
    if (!condivisoGroup) {
      // Trova il parent del checkbox condiviso
      var checkboxCondiviso = document.getElementById('tcondiviso');
      if (checkboxCondiviso) condivisoGroup = checkboxCondiviso.closest('.form-group');
    }
    ...
    var virtualRecoveryGroup = document.querySelector('.form-group:has(#tvirtual)');
    if (!virtualRecoveryGroup) {
      var checkboxVirtual = document.getElementById('tvirtual');
      if (checkboxVirtual) virtualRecoveryGroup = checkboxVirtual.closest('.form-group');
    }
    if (virtualRecoveryGroup) virtualRecoveryGroup.style.display = 'none';
    ...
    if (DB.config && DB.config.mode === 'couple') {
      ...
      if (virtualRecoveryGroup) virtualRecoveryGroup.style.display = 'block';
    }
...
function aggiornaListaObiettiviInvestimento() {
  ...
  DB.obiettivi.forEach(function(obj, index) {
    ...
    html += '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px">';
    html += '<div>';
    html += '<div style="font-weight:700;font-size:1.15em;color:#2c3e50;margin-bottom:4px">' + obj.icona + ' ' + obj.nome + '</div>';
    html += '<div style="font-size:0.85em;color:#7f8c8d">Target: ' + formatEuro(obj.target) + '</div>';
    html += '</div>';
    html += '<div style="text-align:right">';
    html += '<div style="font-size:1.5em;font-weight:900;color:' + borderColor + '">' + formatEuro(obj.risparmiato) + '</div>';
    html += '<div style="font-size:0.75em;color:#7f8c8d;margin-top:2px">' + percentuale.toFixed(1) + '% completato</div>';
    html += '</div>';
    html += '</div>';
    
    // Progress bar
    html += '<div style="background:#ecf0f1;height:10px;border-radius:5px;overflow:hidden;margin-bottom:10px">';
    ...
    html += '<div style="display:flex;justify-content:space-between;font-size:0.8em;color:#7f8c8d">';
    html += '<span>💰 Mancante: <strong style="color:#e65100">' + formatEuro(mancante) + '</strong></span>';
    if (percentuale >= 100) {
      html += '<span style="color:#4caf50;font-weight:700">🎉 OBIETTIVO RAGGIUNTO!</span>';
    }
    html += '</div>';
    html += '</div>';
  });
