...
function vai(sec, elem) {
  ...
  if (sec === 'obiettivi') { aggiornaObiettiviDateHeader(); mostraObiettivi(); aggiornaSaldoCoppiaObiettivi(); }
  if (sec === 'trans') {
    aggFilterCats();
...
function aggiornaCondivisoMensile() {
  ...
  if (daRec > 0.01) {
    ...
    label.textContent = 'Tu Devi al Partner';
...
function mostraLongTermGoals() {
  ...
  DB.longTermGoals.forEach(function(goal, idx) {
    ...
    if (goal.tipo === 'risparmio') {
      ...
      html += '<div class="kpi-box-item c-orange"><div class="kpi-box-label">Mancante</div><div class="kpi-box-val c-orange">' + formatEuro(Math.abs(rimanente)) + '</div></div>';
    } else if (goal.tipo === 'riduzione') {
...
function aggiornaListaObiettiviInvestimento() {
  ...
  DB.obiettivi.forEach(function(obj, index) {
    ...
    html += '<div class="kpi-box-item c-orange"><div class="kpi-box-label">Mancante</div><div class="kpi-box-val c-orange">' + formatEuro(mancante) + '</div></div>';
    html += '</div>';
...
function apriFormRicorrente(idx) {
  ...
  modal.innerHTML = '<div style="background:var(--card);border-radius:24px 24px 0 0;padding:24px 20px 40px;width:100%;max-width:480px;max-height:90vh;overflow-y:auto;-webkit-overflow-scrolling:touch">'
    ...
    + '<input type="month" id="ricDataInizio" value="' + defaultInizio + '" style="' + fs + '"></div>'

    + '<div style="margin-bottom:24px"><label style="' + ls + '">Mese/Anno di fine (lascia vuoto = indefinita)</label>'
    + '<input type="month" id="ricDataFine" value="' + defaultFine + '" style="' + fs + '"></div>'
