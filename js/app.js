...
function mostraTrans() {
  ...
  if (listaFiltrata.length === 0) {
    ...
    dateOrdinate.forEach(function(dataKey) {
      ...
      if (t.virtualRecovery) badges += '<span class="condiviso-badge" style="background:#9b59b6">💝 RECUPERO</span>';

      // ========== LAYOUT BILANCE COMPATTO ==========
      var dotColor = t.tipo === 'income' ? 'var(--income)' : 'var(--expense)';
      var amountColor = t.tipo === 'income' ? 'var(--income)' : (t.tipo === 'partner_payment' ? '#9b59b6' : 'var(--expense)');
      var amountPrefix = t.tipo === 'income' ? '+' : '-';
      var noteText = t.note ? t.note : '';

      var header = document.createElement('div');
      header.className = 'trans-header';
      header.innerHTML = '<div class="trans-dot" style="background:' + dotColor + '"></div>' +
        '<div class="trans-icon">' + icon + '</div>' +
        '<div class="trans-content"><div class="trans-cat">' + t.categoria + badges + '</div>' +
        (noteText ? '<div class="trans-note">' + noteText + '</div>' : '') + '</div>' +
        '<span class="trans-amount" style="color:' + amountColor + '">' + amountPrefix + '€' + t.importo.toFixed(2) + '</span>';
...
function mostraCats() {
  ...
  if (categorieNecessita.length === 0) {
    ...
    categorieNecessita.sort().forEach(function(cat) {
      ...
      htmlNec += '<button onclick="cambiaClassificazione(\'' + cat + '\', \'desideri\')" style="background:#9b59b6;color:#fff;font-size:0.65em;padding:4px 6px;border-radius:4px;border:none;cursor:pointer">→D</button>';
      htmlNec += '</div>';
      htmlNec += '</div>';
    });
  }
  ...
  if (categorieDesideri.length === 0) {
    ...
    categorieDesideri.sort().forEach(function(cat) {
      ...
      htmlDes += '<button onclick="cambiaClassificazione(\'' + cat + '\', \'necessita\')" style="background:#e67e22;color:#fff;font-size:0.65em;padding:4px 6px;border-radius:4px;border:none;cursor:pointer">→N</button>';
      htmlDes += '</div>';
      htmlDes += '</div>';
    });
  }
