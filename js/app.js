...
function aggiornaListaObiettiviModal() {
  ...
  DB.obiettivi.forEach(function(obj, index) {
    ...
    // Card obiettivo con layout pulito
    html += '<div style="background:var(--card);padding:16px;border-radius:14px;margin-bottom:12px;border-left:4px solid ' + progressColor + ';box-shadow:0 2px 8px rgba(0,0,0,0.1)">';

    // Riga header: icona + nome centrati
    html += '<div style="display:flex;flex-direction:column;align-items:center;justify-content:center;margin-bottom:14px">';
    html += '<span style="font-size:2.2em;line-height:1">' + obj.icona + '</span>';
    html += '<span style="font-weight:700;font-size:1.1em;color:var(--text);margin-top:6px;text-align:center">' + obj.nome + '</span>';
    html += '</div>';

    // Info importi centrati
    html += '<div style="display:flex;flex-direction:column;align-items:center;gap:4px;margin-bottom:12px">';
    html += '<div style="font-size:0.85em;color:#7f8c8d">Target: <strong style="color:var(--text)">' + formatEuro(obj.target) + '</strong></div>';
    html += '<div style="font-size:0.95em;font-weight:600;color:' + progressColor + '">' + formatEuro(risparmiato) + ' risparmiati</div>';
    html += '</div>';

    // Barra progresso centrata
    html += '<div style="width:100%;max-width:280px;margin:0 auto 10px auto">';
    html += '<div style="background:var(--border);height:12px;border-radius:6px;overflow:hidden">';
    html += '<div style="background:linear-gradient(90deg,' + progressColor + ',' + progressColor + 'cc);height:100%;width:' + Math.min(percentuale, 100) + '%;border-radius:6px;transition:width 0.3s"></div>';
    html += '</div>';
    html += '<div style="text-align:center;font-size:0.8em;color:' + progressColor + ';font-weight:700;margin-top:4px">' + percentuale + '%</div>';
    html += '</div>';

    // Date centrate
    if (obj.dataInizio || obj.dataFine) {
      html += '<div style="display:flex;flex-direction:column;align-items:center;gap:2px;margin-bottom:12px;font-size:0.8em;color:#888">';
      if (obj.dataInizio) {
        var dInizio = new Date(obj.dataInizio);
    ...
    html += '<button onclick="modificaRisparmiato(' + index + ')" style="background:linear-gradient(135deg,#3498db,#2980b9);color:white;border:none;padding:10px 20px;border-radius:8px;cursor:pointer;font-size:0.85em;font-weight:600">✏️ Modifica</button>';
    html += '<button onclick="eliminaObiettivo(' + index + ')" style="background:#e74c3c;color:white;border:none;padding:10px 16px;border-radius:8px;cursor:pointer;font-size:0.85em">🗑️</button>';
    html += '</div>';
...
function aggiornaListaConti() {
  ...
  DB.contiPersonalizzati.forEach(function(conto, index) {
    ...
    html += '<div class="conto-draggable" draggable="true" data-index="' + index + '" data-id="' + conto.id + '" style="' + borderStyle + ';padding:12px 15px;border-radius:10px;margin-bottom:10px;display:flex;align-items:center;justify-content:space-between;cursor:grab;transition:all 0.2s">';
    // Handle drag
    html += '<div class="drag-handle" style="color:#a1a1aa;font-size:1.2em;margin-right:10px;cursor:grab">⠿</div>';
    ...
    html += '<button onclick="event.stopPropagation();apriModalModificaConto(\'' + conto.id + '\')" style="background:#3498db;color:#fff;border:none;padding:8px 12px;border-radius:6px;cursor:pointer;font-size:0.9em">⚙️</button>';
    html += '<button onclick="event.stopPropagation();eliminaConto(\'' + conto.id + '\')" style="background:#e74c3c;color:#fff;border:none;padding:8px 12px;border-radius:6px;cursor:pointer;font-size:0.9em">🗑️</button>';
    html += '</div>';
    html += '</div>';
  });
  ...
  if (anyoneHasTickets) {
    ...
    html += '<div style="border:2px solid #ff9800;background:rgba(255,152,0,0.05);padding:12px 15px;border-radius:10px;margin-bottom:10px;display:flex;align-items:center;justify-content:space-between">';
    html += '<div style="display:flex;align-items:center;gap:12px;flex:1">';
    html += '<div style="font-size:1.5em">🎟️</div>';
    html += '<div style="flex:1">';
    html += '<div style="font-weight:700;font-size:1em;color:var(--text)">Buoni Pasto</div>';
    html += '<div style="font-size:0.75em;color:#ff9800;margin-top:2px">' + numBuoni + ' buoni × €' + valoreUnitario.toFixed(2) + '</div>';
    html += '</div>';
    html += '<div style="font-weight:800;font-size:1.1em;color:var(--text)">' + formatEuro(saldoBuoni) + '</div>';
    html += '</div>';
    html += '<div style="display:flex;gap:8px;margin-left:15px">';
    html += '<button onclick="apriModalModificaBuoni()" style="background:#ff9800;color:#fff;border:none;padding:8px 12px;border-radius:6px;cursor:pointer;font-size:0.9em">⚙️</button>';
    html += '</div>';
    html += '</div>';
  }
